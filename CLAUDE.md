...
| ~~`doctl` token scoped to wrong account/team~~ — **corrected 2026-08-13**: token/account were always correct; itemized DO invoice history proved this account billed `mu-online-*-production` in `sgp1` through July. Zero droplets meant the resources were destroyed, not wrong-scoped | ✅ | corrected in hub memory `feedback_verify_credential_scope.md` | 2026-08-10 |
...
| Fixed real bug in `terraform/main.tf`: implicit default VPC for `sgp1` no longer exists once region is empty (DO removes it) — replaced `data "digitalocean_vpc"` lookup with an explicit resource, imported after DO auto-recreated one mid-apply. Also fixed image-selection bug: "latest Ubuntu" filter matched DO Marketplace 1-click app images and GPU base images ahead of the real OS image — pinned to `ubuntu-24-04-x64` | ✅ | `terraform` commit `7f84d44` | 2026-08-13 |
| Applied Terraform: recreated all production infra from scratch (web/db/game droplets, VPC, load balancer, DNS records) — 16 resources, 0 destroyed. New IPs: web `159.223.91.156`, db `159.223.71.69`, game `152.42.164.190`, LB `209.38.56.108` | ✅ | DigitalOcean `sgp1`, `terraform.tfstate` | 2026-08-13 |
| Updated `ansible-mu/inventory/hosts.ini` with the new droplet IPs (old ones are dead) | ⬜ | edited locally, **not committed/pushed** — user chose to push + trigger `deploy-production` themselves this session | 2026-08-13 |
| First real CI/CD run since March: `build`/`push` now green; `deploy-production` fails at Ansible SSH — `mu-deploy` key rejected on the web droplet (`159.65.12.125`), matching a previously-flagged-but-unresolved "SSH host-key fingerprints changed" note | ⬜ | run `31279359200` — **still unverified**; the droplet it would target no longer exists (see next row) | 2026-08-09 |
| Built + tested a DigitalOcean cost-check skill (`/do-cost-check`, `scripts/do-cost-check.sh`), wired into `/report` Phase 5 alongside the existing AWS one — used it to confirm the ~$94/mo burn rate below, then to confirm $0.00/h after teardown | ✅ | hub commit `1af13de` (local only — hub repo is archived on GitHub, push blocked); `mu-online-web` commit `ffde14f` | 2026-08-13 |
| Destroyed the just-recreated web/db/game droplets + load balancer (`terraform destroy -target`, 10 resources incl. cascaded DNS records) to stop the ~$94/mo burn rate before the SSH fix was verified — user's call, not a rollback due to failure. Two storage volumes (`mssql_data`, `game_data`, ~$2/mo combined) deliberately left running — both have `prevent_destroy = true` in Terraform and the user chose not to override that guard | ✅ | DigitalOcean `sgp1`; `terraform.tfstate` | 2026-08-13 |
