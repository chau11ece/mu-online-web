...
| First real CI/CD run since March: `build`/`push` now green; `deploy-production` fails at Ansible SSH — `mu-deploy` key rejected on the web droplet (`159.65.12.125`), matching a previously-flagged-but-unresolved "SSH host-key fingerprints changed" note | ⬜ | run `31279359200` — **unverified against the new droplet**; requires the `hosts.ini` push above plus a fresh pipeline run | 2026-08-09 |
