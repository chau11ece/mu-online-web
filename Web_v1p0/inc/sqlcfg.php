...
function clean_post($var=NULL, $is_email = false) {
    // Check if this looks like an email address
    if (!$is_email && strpos($var, '@') !== false && filter_var($var, FILTER_VALIDATE_EMAIL)) {
        $is_email = true;
    }
    
    // For emails, preserve @ symbol and domain
    if ($is_email) {
        return $var;
    }
    
    // Original logic for other fields (accounts, passwords, etc.)
