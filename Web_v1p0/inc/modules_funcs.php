...
function do_pass_reveal(){
    global $option;
    $show_msg = array('error'=> array(), 'success'=> array());
	$email    = clean_post($_POST['email'], true); // true = is_email, preserve @ symbol
	$account  = clean_post($_POST['account']);
	$verify   = clean_post($_POST['verify']);
	
	// Validate inputs
	if(empty($account) OR empty($email)){
	...
	$check = mssql_fetch_array(mssql_query("Select [memb___id], [mail_addr] From [MEMB_INFO] Where [memb___id]='".$account."' and [mail_addr]='".$email."'"));
	...
	$token_expiry = time() + 3600; // 1 hour expiry
	
	// Store token in database (need to add column) or use session
	// For now, we'll generate a new password and send it via email
	$new_password = generate_random_password();
	
	// Update password in database
	$new_password_md5 = ($option['md5'] == 1) ? md5($new_password) : $new_password;
	mssql_query("UPDATE [Memb_Info] SET [memb__pwd] = '".$new_password_md5."' WHERE [memb___id]='".$account."'");
	
	// Try to send email with new password
	if(!empty($option['email_enabled'])){
		include_once 'email_helper.php';
		$result = send_password_reset_email($account, $email, $new_password);
		
		if($result['success']){
			$show_msg['success'][0] = "A new password has been sent to your email address. Please check your inbox.";
		}else{
			// If email fails, show password directly (fallback)
			$show_msg['success'][0] = phrase_acc_pass . "<font color='orange'>" . $new_password . "</font>";
		}
	}else{
...
function do_reset_stats()
	...
	$mail = clean_post($_POST['email'], true); // true = is_email, preserve @ symbol
		...
		// Make security question/answer optional - auto-generate if not provided
		if(empty($sq) OR empty($sa))
		{
			$sq = 'Account Recovery';
			$sa = substr(md5($acc . time()), 0, 8);
		}
		
		// Check required fields (now question/answer are optional)
		if(empty($acc) OR empty($pass) OR empty($repass) OR empty($mail))
		{
			$show_msg['error'][] = phrase_empty_fields;
		}
		elseif(preg_match('/[^a-zA-Z0-9\_\-]/', $acc) OR preg_match('/[^a-zA-Z0-9\_\-]/', $pass) OR preg_match('/[^a-zA-Z0-9\_\-]/', $repass))
		{
			$show_msg['error'][] = phrase_invalid_symbols;
		}
		else
		{
			...
			// Security question/answer validation removed - now optional with auto-generation
			...
			if($error===0)
			{
				...
				else
				{
				    ...
				    if($option['md5'] == 1) {
				    	$do_reg = mssql_query("insert into Memb_Info ([memb___id],[memb__pwd],[memb_name],[sno__numb] ,[mail_addr],[fpas_ques],[fpas_answ],[bloc_code],[ctl1_code],[reg_date],[reg_ip]) Values ('".$acc."',[dbo].[fn_md5]('".$pass."','".$acc."'),'".$acc."','111111111','". $mail ."','". $sq ."','". $sa ."','0','0','".time()."','".ip()."')");
				    } else {
					...
					if(!empty($option['email_enabled'])){
						include_once 'email_helper.php';
						$email_result = send_registration_email($acc, $pass, $mail);
						if($email_result['success']){
							$show_msg['success'][0] = phrase_thanks . $acc . phrase_success_reg . " An email with your account details has been sent to " . $mail;
						}else{
